...
function createQuestionnaireTemplateGenerator() {
     ...
     */
    function ensureAllTargetsExist(routes, filePath = 'unknown') {
    ...
    function getSectionSchemaDefinition(sectionsDirPath, sectionId) {
        ...
        //  const sectionSchemaDefinition = fs.readFileSync(sectionSchemaPath, 'utf8');
        const sectionSchemaDefinition = require(sectionSchemaPath);

        return sectionSchemaDefinition

        //return fs.readFileSync(sectionSchemaPath, 'utf8');;
        ...
        for (let i = 1, len = condition.length, value; i < len; i += 1) {
            ...
            if (typeof value === 'string' && value.startsWith('q-')) {
                ...
                const sectionId = Object.keys(sections).find(
                    sectionId => {
                        if (questionId in sections[sectionId].properties)
                            return sectionId
                    });
                /* eslint-enable no-shadow */
                if (sectionId) {
                    condition[i] = `$.answers.${sectionId}.${questionId}.value`;
                } else {
                    throw new Error(`Question id ("${questionId}") not found in any section` + ` Also: ${sectionId}`);
                }
            } else if (value === 'yes') {
                condition[i] = 'true';
            } else if (value === 'no') {
    ...
    function generate(options) {
        ...
        ensureAllTargetsExist(questionnaireMetadata.routes, questionnaireMetadataFilePath);
