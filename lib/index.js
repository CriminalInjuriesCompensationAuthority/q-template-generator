...
function createQuestionnaireTemplateGenerator() {
    ...
    function getSectionSchemaDefinition(sectionsDirPath, sectionId) {
        ...
        const sectionSchemaDefinition = require(sectionSchemaPath); // eslint-disable-line
    ...
    function processRouteCondition(condition, sections) {
        ...
        for (let i = 1, len = condition.length, value; i < len; i += 1) {
            ...
            if (typeof value === 'string' && value.startsWith('q-')) {
                ...
                const sectionId = Object.keys(sections).find(sectionId => { // eslint-disable-line
                    if (questionId in sections[sectionId].properties) return sectionId;
                });
                ...
                if (sectionId) {
                    ...
                    throw new Error(
                        `Question id ("${questionId}") not found in any section` +
                            ` Also: ${sectionId}`
                    );
                }
            } else if (value === 'yes') {
